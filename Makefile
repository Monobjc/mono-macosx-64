...
MONO_ARCHIVE_URL=$(MONO_URL)/sources/mono/$(MONO_ARCHIVE)
MONO_ARCHIVE_PATH=$(FILES_DIR)/$(MONO_ARCHIVE)
ifeq ($(RELEASE),0)
	MONO_INSTALLER_FILE=MonoFramework-$(KIND)-$(VERSION).macos10.xamarin.x86.dmg
	MONO_INSTALLER_URL=$(MONO_URL)/archive/$(VERSION)/macos-10-x86/$(MONO_PACKAGE)
else
	MONO_INSTALLER_FILE=MonoFramework-$(KIND)-$(VERSION)_$(RELEASE).macos10.xamarin.x86.dmg
	MONO_INSTALLER_URL=$(MONO_URL)/archive/$(VERSION)/macos-10-x86/$(RELEASE)/$(MONO_PACKAGE)
endif
